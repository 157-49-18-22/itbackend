...
router.put('/:id/approve', async (req, res) => {
  try {
    const approval = await db.Approval.findByPk(req.params.id);
    if (!approval) {
    ...
    const updatedApproval = await db.Approval.findByPk(req.params.id, {
      include: [
        { model: db.Project, as: 'project', attributes: ['id', 'name'] },
        { model: db.User, as: 'requestedBy', attributes: ['id', 'name', 'email'] },
        { model: db.User, as: 'requestedTo', attributes: ['id', 'name', 'email'] }
      ]
    });
